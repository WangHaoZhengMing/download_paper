...
impl CosConfig {
    ...
    pub fn new(
        ...
        if let Some(v) = proxies {
            config.proxies = Some(v);
        }
        if let Some(v) = domain {
            config.domain = Some(v);
        }
        if let Some(v) = service_domain {
            config.service_domain = Some(v);
        }
        if let Some(v) = keep_alive {
            config.keep_alive = v;
        }
        if let Some(v) = pool_connections {
            config.pool_connections = v;
        }
        if let Some(v) = pool_maxsize {
            config.pool_maxsize = v;
        }
        if let Some(v) = allow_redirects {
            config.allow_redirects = v;
        }
        if let Some(v) = sign_host {
            config.sign_host = v;
        }
        if let Some(v) = endpoint_ci {
            config.endpoint_ci = Some(v);
        }
        if let Some(v) = endpoint_pic {
            config.endpoint_pic = Some(v);
        }
        if let Some(v) = enable_old_domain {
            config.enable_old_domain = v;
        }
        if let Some(v) = enable_internal_domain {
            config.enable_internal_domain = v;
        }
        if let Some(v) = sign_params {
            config.sign_params = v;
        }
        if let Some(v) = auto_switch_domain_on_retry {
            config.auto_switch_domain_on_retry = v;
        }
        if let Some(v) = verify_ssl {
            config.verify_ssl = Some(v);
        }
        if let Some(v) = ssl_cert {
            config.ssl_cert = Some(v);
        }
        config
    }
}
...
impl CosS3Client {
    ...
    pub async fn upload_file(&self, bucket: &str, local_file_path: &Path, key: &str) -> Result<()> {
        ...
        let secret_id = self
            ...
            .secret_id
            .as_deref()
            .ok_or_else(|| anyhow!("SecretId is required"))?;
        let secret_key = self
            .conf
            .secret_key
            .as_deref()
            .ok_or_else(|| anyhow!("SecretKey is required"))?;

        // 1. SignKey
        let mut mac =
            HmacSha1::new_from_slice(secret_key.as_bytes()).map_err(|e| anyhow!("{}", e))?;
        ...
        // 4. Signature
        let mut mac =
            HmacSha1::new_from_slice(sign_key.as_bytes()).map_err(|e| anyhow!("{}", e))?;
...
use uuid::Uuid;
use crate::modules::credential::FileInfo;

/// 临时凭证数据（简化版，只包含上传所需字段）
#[derive(Debug, Clone)]
...
impl CosUploader {
    /// 从临时凭证创建上传器
    pub fn from_temp_credentials(creds: TempCredentials) -> Self {
        let config = CosConfig::with_temp_credentials(
            creds.region,
            creds.tmp_secret_id,
            creds.tmp_secret_key,
            creds.session_token,
        );
        ...
        Self {
            client,
            bucket: creds.bucket,
            key_prefix: Self::normalize_path(&creds.key_prefix),
            cdn_domain: creds.cdn_domain,
        }
    }

    /// 上传文件并返回文件信息
    pub async fn upload(&self, file_path: &Path) -> Result<FileInfo> {
        let filename = Self::extract_filename(file_path)?;
        let object_key = format!("{}/{}/{}", self.key_prefix, Uuid::new_v4(), filename);

        self.client
            .upload_file(&self.bucket, file_path, &object_key)
            .await?;

        let final_url = format!("https://{}/{}", self.cdn_domain, object_key);
    ...
    fn extract_filename(path: &Path) -> Result<String> {
        path.file_name()
            ...
            .ok_or_else(|| anyhow!("无法从路径中提取文件名: {:?}", path))
    }
}
